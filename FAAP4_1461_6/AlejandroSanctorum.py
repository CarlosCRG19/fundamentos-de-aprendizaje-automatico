    ...
    def validacion(self, particionado, dataset, clasificador, seed=None):
        ...
        for particion in particionado.particiones:
            ...
            clasificador.entrenamiento(
                datostrain, dataset.nominalAtributos, dataset.diccionario
            )
            # Predicting
            pred = clasificador.clasifica(
                datostest, dataset.nominalAtributos, dataset.diccionario
            )
            # Testing error
            ydatos = datostest[:, -1]
...
class ClasificadorNaiveBayes(Clasificador):
    ...
    def _multinomialNB(self, xdata, ydata, feat_idx, diccionario):
        ...
        for value in diccionario[feat_idx]:
            ...
            for class_name in diccionario[-1]:
                ...
                theta_mtx[feat_val_idx][class_idx] = sum(
                    (xdata[:, feat_idx] == feat_val_idx) & (ydata == class_idx)
                ) / sum(ydata == class_idx)
    ...
    def _gaussianNB(self, xdata, ydata, feat_idx, diccionario):
        ...
        # 2 columns: mean and variance for each class
        theta_mtx = np.zeros((n_classes, 2))
        ...
        for class_name in diccionario[-1]:
            ...
            mean_sum = sum(
                elem
                for (idx, elem) in enumerate(xdata[:, feat_idx])
                if ydata[idx] == class_idx
            )
    ...
    def entrenamiento(self, datostrain, atributosDiscretos, diccionario):
        ...
        # initializing array of prior probs with zeros
        self.prior_probs = np.zeros(n_classes)
        for class_name in diccionario[-1]:
            class_idx = diccionario[-1][class_name]
            # P(y=i) = count(ydata==i)/len(ydata)
            self.prior_probs[class_idx] = sum((class_idx == ydata)) / m
            ...
            for k in range(n_classes):
                ...
                for feat_idx in range(n_feat):
                    if atributosDiscretos[feat_idx]:
                        ...
                        class_p *= self.likelihoods[feat_idx][int(xdata[i][feat_idx])][
                            k
                        ]
                    else:
                        ...
                        # calculating posterior probability
                        class_p *= norm.pdf(
                            xdata[i][feat_idx], loc=mean, scale=math.sqrt(var)
                        )
            ...
            for idx_train in range(ntrain):
                if self.dist == "euclidean":
                    ...
                    distances.append(
                        math.sqrt(
                            np.sum(
                                (xtest[idx_test, :] - self.xtrain[idx_train, :]) ** 2
                            )
                        )
                    )
                    ...
                    distances.append(
                        np.sum(
                            np.absolute(xtest[idx_test, :] - self.xtrain[idx_train, :])
                        )
                    )
                    ...
                    distances.append(
                        scipy.spatial.distance.mahalanobis(
                            xtest[idx_test, :], self.xtrain[idx_train, :], self.invSigma
                        )
                    )
                else:
...
class AlgoritmoGenetico(Clasificador):
    def __init__(
        ...
        if add_rule_prob >= 0.5:
            raise ValueError(
                "Parameter 'add_rule_prob' must be in [0, 0.5). It cannot be greater or equal than 0.5"
            )
        else:
    ...
    def __clf_example(individual, example, diccionario):
        ...
        n_feat = (
            len(diccionario) - 1
        )  # number of features. Last dictionary is the class dict
        ...
        for rule in individual:
            ...
            for j in range(n_feat):
                ...
                feat_and = np.bitwise_and(
                    example[cum_idx : cum_idx + len_feat],
                    rule[cum_idx : cum_idx + len_feat],
                )
                if sum(feat_and) == 0:
    ...
    def __fitness(self, individual, xdata, ydata, diccionario):
        ...
        (
            ...
            xdata.shape
        )  #  number of examples and length of each rule (after oneHotEncode)

        n_hits = (
            0  # number of training examples classified correctly by given individual
        )
        ...
        for i in range(n_examples):
            ...
            predicted_class = AlgoritmoGenetico.__clf_example(
                individual, current_example, diccionario
            )
    ...
    def __elitism(self, fitness_list, n_elite_inds=None):
        ...
        while n_extracted < elite_size:
            ...
            if n_extracted == 0:
                ...
                if self.verbose:
                    print("\tBest individual fitness:", best_fitness_val)
    ...
    def __crossover(self, parents, inter_rule_cross=False):
        ...
        for i in range(0, n_inds, 2):
            if i != n_inds - 1:
                ...
                cross = np.random.choice(
                    [1, 0], p=[self.cross_prob, 1 - self.cross_prob]
                )
                if cross:
                    ...
                    # selecting a rule to cross of parent1
                    cross_rule1 = np.random.randint(0, n_rules1)
                    # selecting a rule to cross of parent2
                    cross_rule2 = np.random.randint(0, n_rules2)
                    ...
                    if inter_rule_cross:
                        ...
                        cross_idx = np.random.randint(
                            1, self.rules_len - 1
                        )  # intra-rule crossover
                    # Crossing parents to get descendets
                    child1 = (
                        P1[:cross_rule1]
                        + [P1[cross_rule1][:cross_idx] + P2[cross_rule2][cross_idx:]]
                        + P2[cross_rule2 + 1 :]
                    )
                    child2 = (
                        P2[:cross_rule2]
                        + [P2[cross_rule2][:cross_idx] + P1[cross_rule1][cross_idx:]]
                        + P1[cross_rule1 + 1 :]
                    )
        ...
        for individual in parents:
            for rule in individual:
                for bit in rule:
                    ...
                    bitflip = np.random.choice(
                        [1, 0], p=[self.bitflip_prob, 1 - self.bitflip_prob]
                    )
                    if bitflip:  # flipping
                        if bit == 1:
                            bit = 0
    ...
    def __mutation_add_delete_rule(self, parents):
        ...
        """
        for individual in parents:
            add_del = np.random.choice(
                [1, -1, 0],
                p=[self.add_rule_prob, self.add_rule_prob, 1 - 2 * self.add_rule_prob],
            )
            ...
            # Deleting a random rule from individual
            elif add_del == -1 and len(individual) > 1:
    ...
    def entrenamiento(self, datosTrain, atributosDiscretos, diccionario):
        ...
        for i in range(self.nepochs):
            ...
            fitness_list = self.__calculate_population_fitness(
                xdata, ydata, diccionario
            )
            ...
            if (
                self.cross_strat == "intra" or self.cross_strat == "both"
            ):  # intra-rule crossover
                parents = self.__crossover(parents, inter_rule_cross=False)
            if (
                self.cross_strat == "inter" or self.cross_strat == "both"
            ):  # inter-rule crossover
    ...
    def clasifica(self, datosTest, atributosDiscretos, diccionario):
        ...
        for example in xdata:
            predicted_class = AlgoritmoGenetico.__clf_example(
                self.best_solution, example, diccionario
            )
